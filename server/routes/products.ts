...
router.post('/admin/products', requireAuth, async (req, res) => {
  const {
    ...
    guaranteeDays, features, whatsappMessage, imageBase64
  } = req.body
  ...
  const { rows } = await pool.query(
    `INSERT INTO products (name, service, type, price, account_duration, guarantee_days, features, whatsapp_message, image_base64)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`,
    [name, service || '', type || 'cuenta_completa', price,
     accountDuration || '1 mes', guaranteeDays || 30,
...
router.put('/admin/products/:id', requireAuth, async (req, res) => {
  ...
  const {
    name, service, type, price, accountDuration,
    guaranteeDays, features, whatsappMessage, imageBase64, active
  } = req.body
  ...
  const { rows } = await pool.query(
    `UPDATE products SET
      ...
      guarantee_days=$6, features=$7, whatsapp_message=$8,
